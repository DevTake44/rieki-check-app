...
//
// 2026-09-04変更(5000→10000): v_profit_lines が29万行を超え、5,000件/回だと
// 全件取得に約59回の往復が必要になっていた。DB側(マテリアライズドビュー+
// インデックス済み)は5,000件取得が数ミリ秒と一瞬で、ボトルネックはDBではなく
// この往復回数だったため、経営マトリクスが「読み込み中」のまま止まって見える
// 不具合が発生していた。1回あたりの件数を10,000件に増やして往復回数を約半分
// (59回→約30回)に減らす。10,000件分のJSONサイズは実測で約1.9MBで、Vercel
// Functionsのレスポンス上限4.5MBに対して十分余裕がある。
const LINES_CHUNK_SIZE = 10000;
